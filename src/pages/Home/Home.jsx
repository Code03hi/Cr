...
const Home = () => {
  ...
  const { allCoin, curreny } = useContext(CoinContext)
  ...
  return (
    <div className='home'>
      <div className="hero">
        ...
        <p>Welcome to the world&rsquo;s largest cryptocurrency marketplace. Sign up to explore more about cryptos.</p>
        ...
        </div>
        {
          displayCoin.slice(0, 10).map((item, index) => (
            <Link to={`/coin/${item.id}`} key={index} className="table-layout">
              ...
              <p>{curreny.symbol} {item.current_price.toLocaleString()}</p>
              <p className={item.price_change_24h>0?'green':'red'}>{Math.floor(item.price_change_24h*100)/100}</p>
              <p className='market-cap'>{curreny.symbol} {item.market_cap.toLocaleString()}</p>
            </Link>
          ))
        }
      </div>
